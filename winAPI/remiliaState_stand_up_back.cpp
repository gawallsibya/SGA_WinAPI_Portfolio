...
void remiliaState_stand_up_back_ready::enter(remilia* pCha)
{
...
void remiliaState_stand_up_back_ready::execute(remilia* pCha)
{
	if (!pCha->getAnimation()->isPlay())
	{
		pCha->changeState(new remiliaState_stand_up_back_start);
		return;
	}
}
void remiliaState_stand_up_back_ready::exit(remilia* pCha)
{
...
void remiliaState_stand_up_back_start::enter(remilia* pCha)
{
...
void remiliaState_stand_up_back_start::execute(remilia* pCha)
{
	pCha->getWay() == remilia::WAY_RIGHT ?
...
void remiliaState_stand_up_back_start::exit(remilia* pCha)
{
	pCha->getAnimation()->stop();
	pCha->isArrStateInfo()[remilia::SI_HITDOWN] = false;
	pCha->setAnimation(nullptr);
